...
/**
 * US-001-01-01/02 — Organization Create wizard. Backend-aligned: `POST
 * /organizations` accepts `{name, slug, settings}` only (ORG_WRITE), so the
 * wizard has no admin-user step and always submits with the session Bearer.
 */
export function OrganizationCreateWizard() {
  ...
  const handleConfirm = async () => {
    ...
    try {
      await create.mutateAsync(toCreateRequest(form.getValues()));
